  ...
  try {
    ...
    for (const item of cartProducts) {
      ...
      if (
        ...
        return res.status(400).json({
          success: false,
          error:
            "One or more products in your cart are unavailable or have insufficient stock.",
        });
      }
    }
    let totalAmount = cartData.items.reduce(
    ...
    if (couponCode) {
      ...
      if (!userClaimedCoupon) {
        ...
        return res.status(400).json({
          success: false,
          error: "User has already claimed the coupon.",
        });
      }
    }
  ...
  try {
    ...
    if (hmac == details.payment.razorpay_signature) {
      const order = await Order.findOneAndUpdate(
        ...
        { $set: { status: "placed", paymentId: details.razorpay_payment_id } },
        { new: true }
      );

      console.log(
        "order sending to checkout page from verify payment :",
        order
      );
      // Update stock quantity for each product
      for (const item of cartProducts) {
...
const verifyAddMoneyToWallet = async (req, res) => {
  try {
    ...
    hmac.update(
      details.razorpay.razorpay_order_id +
        "|" +
        details.razorpay.razorpay_payment_id
    );
    hmac = hmac.digest("hex");
